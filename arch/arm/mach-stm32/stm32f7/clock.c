...
#if !defined(CONFIG_STM32_HSI_HZ)
#error "CONFIG_STM32_HSI_HZ not defined!"
#else
#if (CONFIG_STM32_HSI_HZ == 16000000)
...
int configure_clocks(void)
{
	...
	/* Configure for HSI+PLL operation */
	setbits_le32(&STM32_RCC->cr, RCC_CR_HSION);
	while (!(readl(&STM32_RCC->cr) & RCC_CR_HSIRDY))
		;
	...
	writel(sys_pll_psc.pll_m
		...
		&STM32_RCC->pllcfgr);
	clrbits_le32(&STM32_RCC->pllcfgr, RCC_PLLCFGR_PLLSRC); /* HSI as source */
...
static u8 ahb_psc_table[16] = {
	0, 0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 4, 6, 7, 8, 9
};
static u8 apb_psc_table[8] = {
	0, 0, 0, 0, 1, 2, 3, 4
};
...
unsigned long clock_get(enum clock clck)
{
	...
	if ((readl(&STM32_RCC->cfgr) & RCC_CFGR_SWS_MASK) ==
		...
		sysclk = ((CONFIG_STM32_HSI_HZ / pllm) * plln) / pllp;
	}
